...
export function createTableInput(
  schema: Schema,
  forCloudFormation = false
    ...
    AttributeDefinitions: [
      {
        ...
        AttributeType: schema.primaryKey.hash.type.type as ScalarAttributeType,
      },
    ],
    KeySchema: [
      {
  ...
  if (schema.primaryKey.range != null) {
    params.AttributeDefinitions!.push({
      ...
      AttributeType: schema.primaryKey.range.type.type as ScalarAttributeType,
    });
  ...
  if (schema.localSecondaryIndexes.length > 0) {
    params.LocalSecondaryIndexes = schema.localSecondaryIndexes.map((indexMetadata) => {
      ...
      if (
        params.AttributeDefinitions!.find((ad) => indexMetadata.range.name === ad.AttributeName) ==
        null
      ) {
        params.AttributeDefinitions!.push({
          AttributeName: indexMetadata.range.name,
          AttributeType: indexMetadata.range.type.type as ScalarAttributeType,
        });
      }
      ...
      if (indexMetadata.nonKeyAttributes != null && indexMetadata.nonKeyAttributes.length > 0) {
        if (indexMetadata.projection !== "INCLUDE") {
          throw new SchemaError(
            `Invalid configuration for LocalSecondaryIndex ${schema.name}/${indexMetadata.name}. nonKeyAttributes can only be used with projection INCLUDE.`
          );
        }
  ...
  if (schema.globalSecondaryIndexes.length > 0) {
    params.GlobalSecondaryIndexes = schema.globalSecondaryIndexes.map((indexMetadata) => {
      ...
      // make sure this attribute is defined in the AttributeDefinitions
      if (
        params.AttributeDefinitions!.find((ad) => indexMetadata.hash.name === ad.AttributeName) ==
        null
      ) {
        params.AttributeDefinitions!.push({
          AttributeName: indexMetadata.hash.name,
          AttributeType: indexMetadata.hash.type.type as ScalarAttributeType,
        });
      }
      ...
      if (indexMetadata.range != null) {
        ...
        if (
          params.AttributeDefinitions!.find(
            (ad) => (indexMetadata.range as Attribute<any>).name === ad.AttributeName
          ) == null
        ) {
          params.AttributeDefinitions!.push({
            AttributeName: indexMetadata.range.name,
      ...
      const throughput =
        indexMetadata.throughput == null ? schema.throughput : indexMetadata.throughput;
      ...
      if (indexMetadata.nonKeyAttributes != null && indexMetadata.nonKeyAttributes.length > 0) {
        if (indexMetadata.projection !== "INCLUDE") {
          throw new SchemaError(
            `Invalid configuration for GlobalSecondaryIndex ${schema.name}/${indexMetadata.name}. nonKeyAttributes can only be used with projection INCLUDE.`
          );
        }
